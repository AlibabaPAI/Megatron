...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    def trace_handler_factory(dir_name):
        def trace_handler(prof: torch.profiler.profile):
            timestamp = datetime.now().strftime('%Y_%m_%d_%H_%M_%S')
            file_name = f"visual_mem_{timestamp}"
        
            prof.export_chrome_trace(os.path.join(dir_name, f"{file_name}.json"))
            
            prof.export_memory_timeline(os.path.join(dir_name, f"{file_name}_mem.html"))
        return trace_handler

    if torch.distributed.get_rank() == 0 and args.torch_profile:
        current_time = datetime.now().strftime(
                "%Y-%m-%d_%H-%M-%S")
        dir_name = f"./profile/{current_time}"
        os.makedirs(dir_name, exist_ok=True)
        with torch.profiler.profile(
        profile_memory=True,with_stack=True, record_shapes=True,
        schedule=torch.profiler.schedule(wait=10, warmup=2, active=10),
        on_trace_ready=trace_handler_factory(dir_name),
        ) as prof:
